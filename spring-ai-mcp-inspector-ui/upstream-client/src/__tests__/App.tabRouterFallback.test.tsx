// [spring-ai-mcp-inspector PATCH] Tab router fallback tests (#226, t_e8ab0ebb,
// twin of t_f32dedc6). A stale/bookmarked/mistyped hash (e.g. "#elicitation"
// instead of "#elicitations") used to leave the UI dead: no tab with
// aria-selected=true and zero visible tabpanels. These tests pin the
// resilient behavior: every hash resolves to exactly one selected tab and
// one visible panel, with near-miss normalization and a capability-aware
// default fallback. The 1.x line has no Timeline tab, so "timeline" is
// intentionally absent from the matrix.
...
// Every tab id and its trigger's accessible name (UI order). The 1.x line
// has no Timeline tab, so "timeline" is intentionally absent.
...
describe("App - tab router hash fallback (#226)", () => {
  ...
  describe("hashchange after initial load", () => {
    ...
    it("switching between valid hashes keeps exactly one panel", async () => {
      ...
      for (const tabId of ["prompts", "ping", "metadata"]) {
        await setHash(`#${tabId}`);
        await waitFor(() => {
          expectSingleSelectedTab(tabId);
        });
