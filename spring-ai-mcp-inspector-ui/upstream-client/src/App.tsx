...
// [spring-ai-mcp-inspector PATCH] Tab router fallback (#t_f32dedc6):
// When location.hash is empty, unknown, or a near-miss (wrong case or
// singular/plural), fall back to a sensible default so exactly one tab is
// always selected and its tabpanel is visible. Without this, a stale or
// mistyped bookmark produces a dead UI: no tab has aria-selected=true and
// zero tabpanels are rendered.
// Static list of every tab id, used before capabilities are known (initial
// render). The capability-gated correction runs in the serverCapabilities
...
const App = () => {
  ...
  useEffect(() => () => clearTimeout(originatingTabTimerRef.current), []);

  const navigateToOriginatingTab = (originatingTab?: string) => {
    if (!originatingTab) return;

    // [spring-ai-mcp-inspector PATCH] Tab router fallback (#t_f32dedc6):
    // use the shared capability-aware tab list (includes the Timeline tab,
    // which the old inline list omitted) so returning to an originating
    // tab is validated the same way everywhere.
  ...
  const handleResolveElicitation = (
    ...
    setPendingElicitationRequests((prev) => {
      ...
      if (request) {
        ...
        // (including Timeline) and the timer de-dup stay in one place.
        navigateToOriginatingTab(request.originatingTab);
      }
      return prev.filter((r) => r.id !== id);
    });
  ...
  const readResource = async (uri: string) => {
    ...
    // previously-cached resource left the right pane showing stale content.
    // Upstream issue: https://github.com/modelcontextprotocol/inspector/issues/1266
