...
class ProxySessionTest {
	...
	class Accessors {
		...
		void constructor_exposesSuppliedCollaborators() {
			...
			assertThat(session.targetTransport()).isSameAs(transport);
			assertThat(session.browserToTarget()).isSameAs(browserToTarget);
			assertThat(session.targetToBrowser()).isSameAs(targetToBrowser);
	...
	class Close {
		...
		void close_completesSinksAndClosesTransport() {
			...
			StepVerifier.create(browserToTarget.asFlux()).verifyComplete();
		...
		void close_whenTransportCloseFails_swallowsErrorAndStillCloses() {
			...
			when(failing.closeGracefully()).thenReturn(Mono.error(new RuntimeException("close boom")));
