...
class StreamableHttpProxyControllerTest {
	...
	void setUp() {
		...
		controller = new StreamableHttpProxyController(registry, transportFactory, mcpProxy, objectMapper);
	}
	...
	class PostNewSession {
		...
		void postMcp_withoutSessionAndUrl_returns400() throws Exception {
			// given
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"x\"}");

			// when
			ResponseEntity<Object> response = controller.postMcp(null, null, body);
		...
		void postMcp_whenUpstreamConnectFails_returns502() throws Exception {
			// given
			when(transportFactory.openStreamable(any(URI.class))).thenThrow(new RuntimeException("connect failed"));
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"x\"}");

			// when
			ResponseEntity<Object> response = controller.postMcp(null, "http://target/mcp", body);
		...
		void postMcp_newSessionRequest_returnsResponseWithSessionHeader() throws Exception {
			...
			JsonNode response = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"ok\":true}}");
			when(mcpProxy.start(any())).thenAnswer(inv -> {
				ProxySession s = inv.getArgument(0);
			...
			// when
			ResponseEntity<Object> entity = controller.postMcp(null, "http://target/mcp", body);
	...
	class PostExistingSession {
		...
		void postMcp_withUnknownSession_returns404() throws Exception {
			...
			// when
			ResponseEntity<Object> response = controller.postMcp("missing", null, body);
		...
		void postMcp_existingSessionNotification_returns202() throws Exception {
			...
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"method\":\"notifications/x\"}");

			// when
			ResponseEntity<Object> response = controller.postMcp("s1", null, body);
		...
		void postMcp_existingSessionRequest_returnsMatchingResponse() throws Exception {
			...
			ProxySession session = newSession("s1", mock(McpClientTransport.class));
			when(registry.get("s1")).thenReturn(session);
			JsonNode response = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":9,\"result\":{\"v\":1}}");
			session.targetToBrowser().tryEmitNext(response);
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":9,\"method\":\"ping\"}");

			// when
			ResponseEntity<Object> entity = controller.postMcp("s1", null, body);
		...
		void postMcp_existingSessionRequest_skipsNonMatchingFrames() throws Exception {
			...
			ProxySession session = newSession("s1", mock(McpClientTransport.class));
			when(registry.get("s1")).thenReturn(session);
			session.targetToBrowser().tryEmitNext(objectMapper.readTree("\"not-an-object\""));
			session.targetToBrowser().tryEmitNext(objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":99}"));
			session.targetToBrowser().tryEmitNext(objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":null}"));
			JsonNode answer = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":5,\"result\":{\"v\":1}}");
			session.targetToBrowser().tryEmitNext(answer);
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":5,\"method\":\"ping\"}");
		...
		void postMcp_withBlankSessionHeader_opensNewSession() throws Exception {
			...
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStreamable(any(URI.class))).thenReturn(target);
			JsonNode answer = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{}}");
			when(mcpProxy.start(any())).thenAnswer(inv -> {
				ProxySession s = inv.getArgument(0);
			...
			// when
			ResponseEntity<Object> entity = controller.postMcp("   ", "http://target/mcp", body);
		...
		void postMcp_existingSessionNullId_treatedAsNotification() throws Exception {
			...
			ProxySession session = newSession("s1", mock(McpClientTransport.class));
			when(registry.get("s1")).thenReturn(session);
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":null,\"method\":\"x\"}");
		...
		void postMcp_existingSessionNonObjectBody_treatedAsNotification() throws Exception {
			...
			ProxySession session = newSession("s1", mock(McpClientTransport.class));
			when(registry.get("s1")).thenReturn(session);
			JsonNode body = objectMapper.readTree("[1,2,3]");
		...
		void postMcp_notificationEmitFails_returns500() throws Exception {
			...
			when(registry.get("s1")).thenReturn(session);
			JsonNode body = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"method\":\"notify\"}");
	...
	class GetMcp {
		...
		void getMcp_withKnownSession_returnsEmitter() {
			...
			// when
			SseEmitter emitter = controller.getMcp("s1");
		...
		void getMcp_withUnknownSession_returnsErrorEmitter() {
			...
			// when
			SseEmitter emitter = controller.getMcp("missing");
	...
	class DeleteMcp {
		...
		void deleteMcp_whenRemoved_returns200() {
			// given
			when(registry.removeAndClose("s1")).thenReturn(true);

			// when
			ResponseEntity<Void> response = controller.deleteMcp("s1");

			// then
			assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
			verify(registry).removeAndClose("s1");
		}
		...
		void deleteMcp_whenNotRemoved_returns404() {
			// given
			when(registry.removeAndClose("missing")).thenReturn(false);

			// when
			ResponseEntity<Void> response = controller.deleteMcp("missing");
	...
	class Constructor {
		...
		void constructor_withNullMapper_usesDefault() {
			// given / when
			StreamableHttpProxyController c = new StreamableHttpProxyController(registry, transportFactory, mcpProxy,
					null);
