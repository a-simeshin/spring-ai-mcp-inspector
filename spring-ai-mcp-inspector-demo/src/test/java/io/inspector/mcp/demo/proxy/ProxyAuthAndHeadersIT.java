...
class ProxyAuthAndHeadersIT {
	...
	void authOkWithCustomHeadersOpensSession(ProxyAppHarness.Stack stack) throws Exception {
	...
	/**
	 * With auth enabled, omitting the auth token yields 401 even if the client supplies
	 * custom headers — proving auth is not bypassed by header tricks.
	 */
	@ParameterizedTest(name = "{0}")
	@EnumSource(ProxyAppHarness.Stack.class)
	void missingAuthRejectedRegardlessOfCustomHeaders(ProxyAppHarness.Stack stack) throws Exception {
		app = ProxyAppHarness.start(stack, "STREAMABLE", true, AUTH_TOKEN);
		final int port = ProxyAppHarness.port(app);
		final String targetUrl = "http://127.0.0.1:" + port + "/mcp";
		final String proxyBase = "http://127.0.0.1:" + port + "/mcp-inspector-api";
	 ...
	 * builds {@code HttpClientStreamableHttpTransport} from the URL alone, with no API to
	 * pass additional outbound headers. Upstream MCP Inspector supports an
	 * "Authentication" header / arbitrary "Header Name" field in its UI form for exactly
	 * this scenario — we do not yet.
	 *
	 * <p>
	 * This test asserts the current behavior: the session opens successfully (proves the
	 * headers do not crash anything), but we cannot prove the target ever saw them
	 * because the demo MCP server does not expose any tool that reflects request headers,
	 * and the {@code openStreamable} call has no header plumbing. The decision to fix
	 * lives with T31 review; once fixed, this test should be replaced by a positive
	 * forwarding assertion (e.g. add a tiny test-only servlet on the demo that echoes
	 * inbound headers, and assert {@code X-Custom-A: foo} arrives).
	 */
	@ParameterizedTest(name = "{0}")
	@EnumSource(ProxyAppHarness.Stack.class)
	void customHeadersAreNotForwardedToTarget_documentedBug(ProxyAppHarness.Stack stack) throws Exception {
		...
		assertThat(response.statusCode())
			...
			.isEqualTo(200);
		assertThat(response.headers().firstValue("mcp-session-id").orElse(null)).as("session id present on %s", stack)
			.isNotBlank();
	}
