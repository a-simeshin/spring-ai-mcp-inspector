...
class ProxyHandlerTest {
	...
	void setUp() {
		...
		handler = new ProxyHandler(registry, transportFactory, mcpProxy, transportDetector, objectMapper, properties);
	}
	...
	private static Object extractBodyFlux(ServerResponse response) {
	...
	class Health {
		...
		void health_always_returnsOkStatus() {
			// given
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/health").build());

			// when
			ServerResponse response = handler.health(request).block();
	...
	class Config {
		...
		void config_withSseTransport_mapsDefaultsAndServerUrl() {
			// given
			handler.onWebServerStarted(webServerStartedEvent(7000));
		...
		void config_withBlankEndpoint_defaultsServerUrlToMcp() {
			...
			handler.onWebServerStarted(webServerStartedEvent(7000));
			when(transportDetector.detect())
				.thenReturn(new DetectedTransport(TransportType.STREAMABLE, "", null, "WEBFLUX"));
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/config").build());
	...
	class Fetch {
		...
		void fetch_withRealHttpTarget_returnsUpstreamBody() throws Exception {
			...
			server.createContext("/echo", exchange -> {
				byte[] payload = "hello-proxy".getBytes(java.nio.charset.StandardCharsets.UTF_8);
			...
			try {
				...
				ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/fetch")
					.contentType(MediaType.APPLICATION_JSON)
					.body("{\"url\":\"http://127.0.0.1:" + port
							+ "/echo\",\"init\":{\"method\":\"GET\",\"headers\":{\"X-Req\":\"1\"}}}"));

				// when
				ServerResponse response = handler.fetch(request).block();
	...
	class OpenSse {
		...
		void openSse_withSseUrl_registersSessionAndReturnsEventStream() {
			...
			// when
			ServerResponse response = handler.openSse(request).block();
		...
		void openSse_withStreamableHttpType_usesStreamableFactory() {
			// given
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStreamable(any(URI.class))).thenReturn(target);
			ServerRequest request = toServerRequest(
			...
			assertThat(response).isNotNull();
			assertThat(response.statusCode()).isEqualTo(HttpStatus.OK);
			verify(transportFactory).openStreamable(any(URI.class));
		}
		...
		void openSse_streamsPrologueThenMappedFrame() {
			// given
			ProxySession[] captured = new ProxySession[1];
			org.mockito.Mockito.doAnswer(inv -> {
				captured[0] = inv.getArgument(0);
				return null;
			}).when(registry).put(any(ProxySession.class));
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openSse(any(URI.class))).thenReturn(target);
			ServerRequest request = toServerRequest(
					MockServerHttpRequest.get("/mcp-inspector-api/sse?transportType=sse&url=http://up/sse").build());
			...
			ServerResponse response = handler.openSse(request).block();
			JsonNode frame = objectMapper.createObjectNode().put("hello", "world");
			...
			// event
			@SuppressWarnings("unchecked")
			reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>> body = (reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>>) extractBodyFlux(
					response);
		...
		void openStdio_withoutCommand_returnsBadRequest() {
			// given
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/stdio").build());

			// when
			ServerResponse response = handler.openStdio(request).block();
		...
		void openStdio_withCommand_registersSessionAndReturnsEventStream() {
			// given
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStdio(any(), any())).thenReturn(target);
			URI uri = URI.create("/mcp-inspector-api/stdio?command=node&args=server.js&env=%7B%22A%22%3A%221%22%7D");
			ServerRequest request = toServerRequest(MockServerHttpRequest.method(HttpMethod.GET, uri).build());
		...
		void openStdio_withMalformedEnv_fallsBackToEmptyEnv() {
			...
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStdio(any(), any())).thenReturn(target);
			URI uri = URI.create("/mcp-inspector-api/stdio?command=node&env=not-json");
			ServerRequest request = toServerRequest(MockServerHttpRequest.method(HttpMethod.GET, uri).build());
			...
			assertThat(response).isNotNull();
			assertThat(response.statusCode()).isEqualTo(HttpStatus.OK);
			verify(transportFactory).openStdio(any(), org.mockito.ArgumentMatchers.eq(Map.of()));
		}
	...
	class PostMessage {
		...
		void postMessage_withoutSessionId_returnsBadRequest() {
			// given
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/message")
				.contentType(MediaType.APPLICATION_JSON)
				.body("{}"));

			// when
			ServerResponse response = handler.postMessage(request).block();
		...
		void postMessage_withKnownSession_emitsFrameAndReturnsAccepted() {
			// given
			ProxySession session = newSession("s1");
		...
		void postMessage_whenEmitFails_returns500() {
			...
			// emits fail
			ProxySession session = newSession("s-fail");
			session.browserToTarget().tryEmitComplete();
			when(registry.get("s-fail")).thenReturn(session);
			ServerRequest request = toServerRequest(
	...
	class PostMcp {
		...
		void postMcp_newSessionWithoutUrl_returnsBadRequest() {
			// given
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
				.contentType(MediaType.APPLICATION_JSON)
				.body("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"ping\"}"));

			// when
			ServerResponse response = handler.postMcp(request).block();
		...
		void postMcp_newSessionWhenUpstreamConnectFails_returns502() {
			// given
			when(transportFactory.openStreamable(any(URI.class))).thenThrow(new RuntimeException("connection refused"));
			ServerRequest request = toServerRequest(
		...
		void postMcp_requestToKnownSession_returnsMatchingUpstreamResponse() {
			...
			ProxySession session = newSession("s-known");
			when(registry.get("s-known")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
			...
			ServerResponse response = handler.postMcp(request).doOnSubscribe(s -> new Thread(() -> {
				try {
					Thread.sleep(50);
				}
				...
				JsonNode reply = objectMapper.createObjectNode().put("jsonrpc", "2.0").put("id", 99);
		...
		void postMcp_notificationToKnownSession_returnsAccepted() {
			// given
			ProxySession session = newSession("s2");
			when(registry.get("s2")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
		...
		void postMcp_newSessionRequest_awaitsAndReturnsUpstreamResponse() {
			...
			when(transportFactory.openStreamable(any(URI.class))).thenReturn(target);
			ProxySession[] captured = new ProxySession[1];
			when(registry.get(any())).thenAnswer(inv -> captured[0]);
			org.mockito.Mockito.doAnswer(inv -> {
				captured[0] = inv.getArgument(0);
				return null;
			}).when(registry).put(any(ProxySession.class));
			...
			Mono<ServerResponse> mono = handler.postMcp(request);
			ServerResponse response = mono.doOnSubscribe(s -> {
				try {
					JsonNode reply = objectMapper.readTree("{\"jsonrpc\":\"2.0\",\"id\":42,\"result\":{\"ok\":true}}");
	...
	class GetDeleteMcp {
		...
		void getMcp_withUnknownSession_returnsNotFound() {
			// given
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/mcp").build());

			// when
			ServerResponse response = handler.getMcp(request).block();
		...
		void getMcp_withKnownSession_returnsEventStream() {
			// given
			ProxySession session = newSession("s3");
			when(registry.get("s3")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/mcp")
		...
		void deleteMcp_whenSessionRemoved_returnsOk() {
			// given
			when(registry.removeAndClose("s4")).thenReturn(true);
			ServerRequest request = toServerRequest(MockServerHttpRequest.delete("/mcp-inspector-api/mcp")
				.header(ProxyConstants.MCP_SESSION_ID_HEADER, "s4")
				.build());

			// when
			ServerResponse response = handler.deleteMcp(request).block();
		...
		void deleteMcp_whenSessionUnknown_returnsNotFound() {
			// given
			when(registry.removeAndClose(any())).thenReturn(false);
			ServerRequest request = toServerRequest(MockServerHttpRequest.delete("/mcp-inspector-api/mcp").build());
	...
	class ConstructorAndConfigEdges {
		...
		void openSse_withFiveArgConstructor_usesLegacyProxyBaseInPrologue() {
			...
			ProxyHandler fiveArg = new ProxyHandler(registry, transportFactory, mcpProxy, transportDetector, null);
			ProxySession[] captured = new ProxySession[1];
			org.mockito.Mockito.doAnswer(inv -> {
				captured[0] = inv.getArgument(0);
				return null;
			...
			// when
			ServerResponse response = fiveArg.openSse(request).block();
			captured[0].targetToBrowser().tryEmitComplete();
		...
		void config_withNullTransportType_mapsEmptyTransport() {
			...
			handler.onWebServerStarted(webServerStartedEvent(7000));
			when(transportDetector.detect()).thenReturn(new DetectedTransport(null, "/mcp", null, "WEBFLUX"));
			ServerRequest request = toServerRequest(MockServerHttpRequest.get("/mcp-inspector-api/config").build());
	...
	class FetchBranches {
		...
		void fetch_withPostBodyHeadersAndNon2xxStatus_mapsOkFalse() throws Exception {
			...
			server.createContext("/notfound", exchange -> {
				byte[] payload = "nope".getBytes(java.nio.charset.StandardCharsets.UTF_8);
	...
	class OpenBlankInputBranches {
		...
		void openStdio_withBlankCommand_returnsBadRequest() {
			...
			URI uri = URI.create("/mcp-inspector-api/stdio?command=%20%20");
			ServerRequest request = toServerRequest(MockServerHttpRequest.method(HttpMethod.GET, uri).build());
		...
		void openStdio_withBlankArgs_usesCommandOnly() {
			...
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStdio(any(), any())).thenReturn(target);
			URI uri = URI.create("/mcp-inspector-api/stdio?command=node&args=%20");
			ServerRequest request = toServerRequest(MockServerHttpRequest.method(HttpMethod.GET, uri).build());
			...
			assertThat(response).isNotNull();
			assertThat(response.statusCode()).isEqualTo(HttpStatus.OK);
			verify(transportFactory).openStdio(org.mockito.ArgumentMatchers.eq(java.util.List.of("node")),
		...
		void openStdio_withBlankEnv_fallsBackToEmptyEnv() {
			...
			McpClientTransport target = mock(McpClientTransport.class);
			when(transportFactory.openStdio(any(), any())).thenReturn(target);
			URI uri = URI.create("/mcp-inspector-api/stdio?command=node&env=%20");
			ServerRequest request = toServerRequest(MockServerHttpRequest.method(HttpMethod.GET, uri).build());
	...
	class PostMcpBranches {
		...
		void postMcp_newSessionWithBlankUrl_returnsBadRequest() {
			...
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp?url= ")
				.contentType(MediaType.APPLICATION_JSON)
				.body("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"ping\"}"));
		...
		void postMcp_frameWithNullId_treatedAsNotification() {
			...
			ProxySession session = newSession("s-null-id");
			when(registry.get("s-null-id")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
		...
		void postMcp_nonObjectFrame_treatedAsNotification() {
			...
			ProxySession session = newSession("s-array");
			when(registry.get("s-array")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
		...
		void postMcp_notificationWhenEmitFails_returns500() {
			...
			ProxySession session = newSession("s-emit-fail");
			session.browserToTarget().tryEmitComplete();
			when(registry.get("s-emit-fail")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
		...
		void postMcp_skipsNonMatchingFramesThenReturnsMatch() {
			...
			ProxySession session = newSession("s-skip");
			when(registry.get("s-skip")).thenReturn(session);
			ServerRequest request = toServerRequest(MockServerHttpRequest.post("/mcp-inspector-api/mcp")
			...
			ServerResponse response = handler.postMcp(request).doOnSubscribe(s -> new Thread(() -> {
				...
				// wrong id (matchesId false), an array (matchesId not-object false), then
				// the genuine matching reply
				session.targetToBrowser().tryEmitNext(objectMapper.createObjectNode().put("id", 999));
				session.targetToBrowser().tryEmitNext(objectMapper.createArrayNode().add("x"));
				session.targetToBrowser()
					.tryEmitNext(objectMapper.createObjectNode().put("jsonrpc", "2.0").put("id", 7));
			}).start()).block();
	...
	private static ProxySession newSession(String id) {
		McpClientTransport transport = mock(McpClientTransport.class);
		when(transport.closeGracefully()).thenReturn(Mono.empty());
		reactor.core.publisher.Sinks.Many<JsonNode> b2t = reactor.core.publisher.Sinks.many()
			.unicast()
			.onBackpressureBuffer();
		reactor.core.publisher.Sinks.Many<JsonNode> t2b = reactor.core.publisher.Sinks.many().replay().limit(256);
