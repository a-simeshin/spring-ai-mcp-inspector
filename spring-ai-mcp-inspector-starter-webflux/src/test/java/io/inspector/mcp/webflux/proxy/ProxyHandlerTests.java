...
class ProxyHandlerTests {
	...
	@Nested
	@DisplayName("postMcp(): connection timeout budget")
	class ConnectionTimeoutBudget {

		@Test
		@Story("Connection timeout")
		@Severity(SeverityLevel.CRITICAL)
		@Description("postMcp() with connectionTimeout=1s times out on the initialize phase when the upstream never responds, returning the structured connection_timeout payload")
		void postMcp_initializePhaseTimeout_returnsConnectionTimeoutPayload() throws Exception {
			// given: a transport that builds but whose proxy never emits a matching
			// response, and a 1s budget override
			final McpClientTransport target = mock(McpClientTransport.class);
			given(ProxyHandlerTests.this.transportFactory.openStreamable(any(URI.class))).willReturn(target);
			given(ProxyHandlerTests.this.mcpProxy.start(any())).willReturn(Mono.empty());
			final ServerRequest request = toServerRequest(
					MockServerHttpRequest.post("/mcp-inspector-api/mcp?connectionTimeout=1")
						.contentType(MediaType.APPLICATION_JSON)
						.body("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\"}"));

			// when: the 1s budget applies to the initialize phase
			final ServerResponse response = ProxyHandlerTests.this.handler.postMcp(request).block();

			// then: the structured connection_timeout payload names the phase and the
			// numbers
			assertThat(response).isNotNull();
			assertThat(response.statusCode()).isEqualTo(HttpStatus.GATEWAY_TIMEOUT);
			@SuppressWarnings("unchecked")
			final Map<String, Object> bodyMap = (Map<String, Object>) ((EntityResponse<Object>) response).entity();
			@SuppressWarnings("unchecked")
			final Map<String, Object> error = (Map<String, Object>) bodyMap.get("error");
			assertThat(error.get("code")).isEqualTo("connection_timeout");
			assertThat(error.get("phase")).isEqualTo("initialize");
			assertThat(((Number) error.get("budgetMs")).longValue()).isEqualTo(1000L);
			assertThat(((Number) error.get("elapsedMs")).longValue()).isGreaterThanOrEqualTo(900L);
			assertThat(error.get("retryable")).isEqualTo(Boolean.TRUE);
			assertThat((String) error.get("message")).contains("initialize").contains("1000ms");
			verify(ProxyHandlerTests.this.registry, org.mockito.Mockito.atLeastOnce())
				.removeAndClose(any(String.class));
		}

		@Test
		@Story("Connection timeout")
		@Severity(SeverityLevel.CRITICAL)
		@Description("postMcp() without connectionTimeout preserves the legacy MCP_CONNECT_FAILED timeout behavior")
		void postMcp_withoutConnectionTimeout_preservesLegacyTimeoutBehavior() throws Exception {
			// given: a short request timeout so the await fails fast, and a transport
			// that builds but whose proxy never emits a matching response
			...
			final ProxyHandler shortTimeoutHandler = new ProxyHandler(ProxyHandlerTests.this.registry,
					ProxyHandlerTests.this.transportFactory, ProxyHandlerTests.this.mcpProxy,
					ProxyHandlerTests.this.transportDetector, ProxyHandlerTests.this.objectMapper, props);
			final McpClientTransport target = mock(McpClientTransport.class);
			given(ProxyHandlerTests.this.transportFactory.openStreamable(any(URI.class))).willReturn(target);
			...
			// when: no connectionTimeout parameter is given
			final ServerResponse response = shortTimeoutHandler.postMcp(request).block();

			// then: the legacy MCP_CONNECT_FAILED shape is preserved
			assertThat(response).isNotNull();
			assertThat(response.statusCode()).isEqualTo(HttpStatus.GATEWAY_TIMEOUT);
			@SuppressWarnings("unchecked")
			final Map<String, Object> bodyMap = (Map<String, Object>) ((EntityResponse<Object>) response).entity();
			@SuppressWarnings("unchecked")
			final Map<String, Object> error = (Map<String, Object>) bodyMap.get("error");
			assertThat(error.get("code")).isEqualTo("MCP_CONNECT_FAILED");
			assertThat(error.get("reason")).isEqualTo("timeout");
			assertThat(error.get("retryable")).isEqualTo(Boolean.TRUE);
			verify(ProxyHandlerTests.this.registry, org.mockito.Mockito.atLeastOnce())
				.removeAndClose(any(String.class));
		}
