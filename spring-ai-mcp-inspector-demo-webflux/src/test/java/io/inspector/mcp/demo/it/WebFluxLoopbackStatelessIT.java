...
class WebFluxLoopbackStatelessIT {
	...
	@BeforeEach
	void connect() {
		client = loopbackFactory.forStateless("127.0.0.1", port, "/mcp");
		client.initialize();
	}
	...
	@Test
	@DisplayName("readResource via the reactive loopback stateless client returns the demo greeting payload")
	@Story("Reactive loopback stateless resources/read")
	@Severity(SeverityLevel.CRITICAL)
	@Description("Reads demo://greeting over the reactive loopback stateless transport and asserts the exact text "
			+ "DemoResourcesProvider serves, so a transport that returns an empty or truncated body still fails.")
	void readResource_viaReactiveLoopbackStateless_returnsGreetingPayload() {
		// when
	...
	@Test
	@DisplayName("callTool via the reactive loopback stateless client executes currentTime")
	@Story("Reactive loopback stateless tools/call")
	@Severity(SeverityLevel.CRITICAL)
	@Description("Calls the argument-less currentTime tool over the reactive loopback stateless transport and asserts "
	...
	void callTool_viaReactiveLoopbackStateless_returnsFreshInstant() {
		// given
