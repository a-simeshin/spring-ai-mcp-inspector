...
class WebFluxLoopbackStreamableIT {
	...
	@Test
	@DisplayName("readResource via the reactive loopback streamable-HTTP client returns the demo greeting payload")
	@Story("Reactive loopback streamable resources/read")
	@Severity(SeverityLevel.CRITICAL)
	@Description("Reads demo://greeting over the reactive loopback streamable-HTTP transport and asserts the exact text "
			+ "DemoResourcesProvider serves, so a transport that returns an empty or truncated body still fails.")
	void readResource_viaReactiveLoopbackStreamable_returnsGreetingPayload() {
		// when
	...
	@Test
	@DisplayName("callTool via the reactive loopback streamable-HTTP client executes currentTime")
	@Story("Reactive loopback streamable tools/call")
	@Severity(SeverityLevel.CRITICAL)
	@Description("Calls the argument-less currentTime tool over the reactive loopback streamable-HTTP transport and asserts "
	...
	void callTool_viaReactiveLoopbackStreamable_returnsFreshInstant() {
		// given
