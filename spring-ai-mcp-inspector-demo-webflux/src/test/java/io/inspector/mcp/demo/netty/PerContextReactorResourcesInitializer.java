 ...
 *
 * <p>
 * Spring Boot's autoconfigured {@code reactorResourceFactory} defaults to
...
public class PerContextReactorResourcesInitializer implements ApplicationContextInitializer<GenericApplicationContext> {
	...
	/**
	 * The bean name Spring Boot's own definition uses (Boot 4.1 renamed it from
	 * {@code reactorServerResourceFactory} when it moved to the
	 * {@code spring-boot-reactor-netty} module). Its {@code @ConditionalOnMissingBean}
	 * matches by type, so the name is not what makes Boot back off — but matching it
	 * keeps the server and the WebClient connector sharing one factory the way they
	 * normally do, and keeps anything injecting by name working.
	 */
	private static final String BEAN_NAME = "reactorResourceFactory";
