...
class ProxyTargetLossIT {
	...
	void targetLossMidSession_whenUpstreamKilled_surfacesAsErrorAndCleansUp() throws Exception {
		...
		// HttpTimeoutException). On the WEBFLUX ProxyAppHarness.stack() the upstream
		// target's
		...
		// On the WEBFLUX ProxyAppHarness.stack(), killing the upstream disposes the
		// JVM-shared
		...
		// (or a ConnectException on the reactive ProxyAppHarness.stack(), per the note
		// above).
		try {
